...
{
  if (s_current_protocol[INTERNAL_MODULE] == PROTO_PXX || s_current_protocol[INTERNAL_MODULE] == PROTO_MULTIMODULE) {
    ...
    else if(s_current_protocol[INTERNAL_MODULE] == PROTO_MULTIMODULE) {
      DMA_InitStructure.DMA_Memory0BaseAddr = CONVERT_PTR_UINT(modulePulsesData[INTERNAL_MODULE].dsm2.pulses);
      DMA_InitStructure.DMA_BufferSize = modulePulsesData[INTERNAL_MODULE].dsm2.ptr - modulePulsesData[INTERNAL_MODULE].dsm2.pulses;
    }
